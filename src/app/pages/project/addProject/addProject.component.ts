...
export class AddProjectComponent implements OnInit {
  ...
  patchProjectForm(project: any): void {
    ...
    this.selectedSkill = project.skills.split(', ')
    this.selectedSkill = this.selectedSkill.map(oi => Number(oi) )
    ...
    this.projectForm.patchValue({
      ...
      project_type: (project.isPrivate ? 'is_private' : project.isHiddenBid ? 'is_hidded_bid' :
        'is_urgent')
    });
  }
  ...
  getProjectFormData(): any {
    ...
    const project: any = {
      ...
      isPrivate: this.projectForm.value.project_type == 'is_private',
      isHiddenBid: this.projectForm.value.project_type == 'is_hidded_bid',
      isFeatured: this.projectForm.controls.is_featured.value,
      isUrgent: this.projectForm.value.project_type == 'is_urgent',
      priceRangeId: this.projectForm.controls.project_range_id.value,
      status: 0,
      skills: this.selectedSkill.join(', ')
    };
    if (this.action === 'edit') {
    ...
    this.customService.post(project, 'project').subscribe(res => {
      if (res) {
        if (res.code <= 300) {
          ...
          this.router.navigate(['/hireExpert'])
          // {queryParams: {qsServiceID: project.serviceId, qsProjectId: res.projectId}} => add this if changes asked
          }else {
